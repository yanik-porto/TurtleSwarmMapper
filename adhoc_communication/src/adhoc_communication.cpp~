...
/*Tsm function*/
bool sendPointCloud(adhoc_communication::SendPointCloud::Request &req,
        adhoc_communication::SendPointCloud::Response & res)
{
    /* Description:
    ...
    ROS_DEBUG("Service called to QUATERNION...");
    string s_msg = getSerializedMessage(req.cloud);
    /*Call the function sendPacket and with the serialized object and the frame payload type as parameter*/
    res.status = sendPacket(req.dst_robot, s_msg, FRAME_DATA_TYPE_POINTCLOUD, req.topic);

#ifdef PERFORMANCE_LOGGING_SERVICE_CALLS
    Logging::logServiceCalls("SendPointCloud", time_call, getMillisecondsTime(), s_msg.size(), res.status);
#endif
    return true;
}
...
int main(int argc, char **argv)
{
    ...
    /*Tutorial*/
    ros::ServiceServer sendQuaternionS = n_pub->advertiseService(robot_prefix + node_prefix + "send_quaternion", sendQuaternion);
